 ...
 *  @author Kevin Wayne
 */
public class IngredientHash<Key, Value> {
    ...
    private void resize(int chains) {
        IngredientHash<Key, Value> temp = new IngredientHash<Key, Value>(chains);
        for (int i = 0; i < m; i++) {
    ...
    private int hash(Key key) {
    	...
    	for (int i = 0; i < ((String) key).length(); i++)
    	    hash = (23 * hash + ((String) key).charAt(i)) % m;
		return hash;
    } 
