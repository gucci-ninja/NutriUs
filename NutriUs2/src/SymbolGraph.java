...
public class SymbolGraph {
    	...
    	for (int i = 0; i < userlist.length; i++) {
        	if (sg.contains(userlist[i])){
        		for (int x : graph.adj(sg.indexOf(userlist[i]))) {
        			...
        			int size = 0;
        			for (int j: graph.adj(x)) {
        				size+=1;
        				//System.out.println(sg.nameOf(j));
        			}
        			//while(graph.adj(x).iterator().hasNext()) {
        				//size+=1;
        				//System.out.println(graph.adj(x).iterator().next());
        			//}
        			if (!potential_recipes.contains(sg.nameOf(x)) && size <= userlist.length)
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        // User List
        String[] userlist = {"pinto beans", "allpurpose flour", "salt  pepper", "salmon", "porkchops", "potatoes"};
